((a,b,c)=>{a[b]=a[b]||{}
a[b][c]=a[b][c]||[]
a[b][c].push({p:"main.dart.js_44",e:"beginPart"})})(self,"$__dart_deferred_initializers__","eventLog")
$__dart_deferred_initializers__.current=function(a,b,c,$){var A,D,B={Ow:function Ow(d,e,f,g){var _=this
_.c=d
_.d=e
_.a=f
_.b=g},AS:function AS(d){this.a=d}},C,F,E
A=c[0]
D=c[2]
B=a.updateHolder(c[45],B)
C=c[64]
F=c[84]
E=c[96]
B.Ow.prototype={
K(){return"AdSize."+this.b}}
B.AS.prototype={
awd(){},
B(d){var x=null
return new A.aR(F.db,new A.aw(x,10,A.e_(A.b([C.a9Q,A.ka(x,x,C.Ut,x,x,this.gawc(),D.S,x,x)],y.a),D.cp,x,D.cR,D.X,x),x),x)}}
var z=a.updateTypes(["~()"]);(function installTearOffs(){var x=a._instance_0u
x(B.AS.prototype,"gawc","awd",0)})();(function inheritance(){var x=a.inherit
x(B.Ow,A.iM)
x(B.AS,A.R)})()
A.bL(b.typeUniverse,JSON.parse('{"AS":{"R":[],"c":[]}}'))
var y={a:A.Q("r<c>")};(function constants(){C.EE=new B.AS(null)
C.EF=new B.Ow(300,600,3,"halfPage")
C.LG=new A.ai(1,3,1,1)
C.Ut=new A.dG(D.qK,10,E.kA,null,null,null)
C.a8B=new A.q(!0,E.kA,null,"Montserrat",null,null,8,D.b8,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null,null)
C.a9Q=new A.dQ("AdSense",null,C.a8B,D.aL,null,null,null,null,null,null)})()};
((a,b)=>{a[b]=a.current
a.eventLog.push({p:"main.dart.js_44",e:"endPart",h:b})})($__dart_deferred_initializers__,"N2EkItJLRTbOnjw+/uBrkC0gCZQ=");